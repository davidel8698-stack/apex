...
## Command-Invoked / Event-Triggered (23)
...
| `subagent-stop.sh` | SubagentStop event (auto-wired R4-007, Claude Code runtime) | Subagent lifecycle cleanup; reads agent_name from stdin JSON. **Campaign B B2.1 extension:** resolves the matching `subagent_start` via envelope `.agent_id` or in-flight registry lookup, denormalises agent-attributed events into `.apex/subagent-transcripts/<agent_name>-<round_tag>-<sha1_8>.jsonl`, emits `subagent_stop` + `transcript_imported` boundary events, flips the registry to `status=stopped` (atomic tmp+mv). |
| `event-log-rotate.sh` | SessionStart event (auto-wired Campaign B B2.3) | Rotates `.apex/event-log.jsonl` when size ≥ 10 MB to `event-log-<YYYY-MM-DD>.jsonl.gz`; jointly rotates aged subagent-transcripts/ entries (≥ 30 days) into `subagent-transcripts-<YYYY-MM-DD>.tar.gz`; prunes any rotated archive ≥ 90 days. Closes GAP-2 (cross-session aggregation via dated archives) + GAP-4 (unbounded log growth). Emits `log_rotated` event on actual movement; silent no-op otherwise. Exit 0 always. |
